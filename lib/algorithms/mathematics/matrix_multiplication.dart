...
List<List<num>> dotProduct(List<List<num>> A, List<List<num>> B) {
  ...
  final matrix = List<List<num>>.generate(A.length, (index) => List<num>.generate(B.first.length, (_) => 0));
  for (var x = 0; x < A.length; x++) {
    for (var y = 0; y < B.first.length; y++) {
      matrix[x][y] = oneDimensionalMultiplication(A[x], List.generate(B.length, (index) => B[index][y]));
